...
export const INSTRUCTIONS = [
  ...
  "9. AT EVERY SESSION START — check cron-job health. Run `node .llm-wiki-memory/src/scripts/cli.mjs cron-health` (or shell-out equivalent). If the response has `healthy: false`, the last hourly cron attempt errored and no later attempt cleared it. Surface the message to the user and ask if they want to investigate. (Claude Code does this automatically via the SessionStart hook; non-Claude clients must run it manually — one-shot, cheap.) Self-healing principle: never silently swallow cron failures; either the next tick clears them or the user is told.",
