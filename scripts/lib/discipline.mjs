...
export const INSTRUCTIONS = [
  ...
  "10. TOPOLOGY CATEGORIES NEED AN EXPLICIT PATH. A category with a `topology:` block in `.layout/layout.yaml` (e.g. tracker `issues`) nests by a path-compiler, NOT by facet placement — so you MUST pass `path=`. Before saving, READ `.layout/layout.yaml`: find the category's `file_kinds` and pick the one matching your intent (a tracker PLAN vs a tracker KNOWLEDGE leaf), then compute `path` from that file_kind's required facets (issues plan -> `issues/<tracker>/<prefix>/<thousands>/<hundredsTens>/<units>/<lifecycle>/<PREFIX>-<N>-<slug>.plan.md`; issues knowledge omits the `<lifecycle>` segment). Omitting `path`, or passing one that does not match the topology, is REFUSED by the server (deterministic) — it would otherwise land flat at the category root, unsearchable and frozen out of lifecycle moves.",
  "Categories: knowledge, self_improvement, plans, investigations, daily (+ any extra category declared in .layout/layout.yaml, e.g. a tracker `issues` topology). Fuller rules live in .agents/rules/ (mirrored to .claude/skills/ and .cursor/rules/).",
].join("\n");
