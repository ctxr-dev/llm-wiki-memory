...
test("inferFacets: a real provided area is kept; valid atom_type kept", () => {
  const p = facets.inferFacets({ category: "knowledge", meta: { area: "frontend", atom_type: "decision" } });
...
test("inferFacets: project_module (a sub-module, not the workspace) becomes the area", () => {
  const p = facets.inferFacets({ category: "knowledge", meta: { project_module: "billing", atom_type: "reference" } });
  assert.equal(p.area, "billing", "any non-workspace project_module is accepted as the sub-module, even without an existing folder");
});

test("inferFacets: the workspace name is never used as an area", () => {
  const p = facets.inferFacets({ category: "knowledge", meta: { project_module: "testproj", atom_type: "reference" } });
...
test("inferFacets: unknown/unscoped area resolves to the cross-cutting fallback, never unknown", () => {
  for (const bad of ["unknown", "unscoped", "", "  "]) {
    const p = facets.inferFacets({ category: "knowledge", meta: { area: bad, atom_type: "reference" } });
...
test("inferFacets: an out-of-set knowledge atom_type is corrected to a valid type", () => {
  for (const bad of ["knowledge", "investigation", "", "made-up"]) {
    const p = facets.inferFacets({ category: "knowledge", meta: { area: "frontend", atom_type: bad } });
    assert.ok(facets.validAtomTypes("knowledge").has(p.atom_type), `'${bad}' -> '${p.atom_type}' must be a valid knowledge atom_type`);
  }
});
...
test("inferFacets: a tag that names a known sub-module is used when area is absent", () => {
  ...
  const p = facets.inferFacets({ category: "knowledge", meta: { atom_type: "reference" }, tags: "frontend,ui" });
  assert.equal(p.area, "frontend");
});
...
test("facetIssues: flags bad area and out-of-set knowledge atom_type only", () => {
  assert.deepEqual(facets.facetIssues("knowledge", { area: "unknown", atom_type: "reference" }), ["area"]);
  assert.deepEqual(facets.facetIssues("knowledge", { area: "testproj", atom_type: "reference" }), ["area"]);
  assert.deepEqual(facets.facetIssues("knowledge", { area: "frontend", atom_type: "knowledge" }), ["atom_type"]);
  assert.deepEqual(facets.facetIssues("knowledge", { area: "frontend", atom_type: "reference" }), []);
  assert.deepEqual(facets.facetIssues("self_improvement", { area: "frontend", task_type: "unknown" }), [], "task_type unknown is not flagged (valid sentinel)");
