...
**Writes default to your brain; a shared repo is opt-in.** `scopes` says which wikis a call concerns; the optional `target` says which one a write goes into. Every write and every document mutation lands in your private memory tree unless you pass `target=<a resolved level>` (its wiki root or mount directory, or `"brain"`). The engine never writes to a shared repo (a mount inside a project checked into git) unless `target` names it, so a shared repo merely being in `scopes` still routes the write to the brain. A shared write only stages the leaf in that repo's working tree and runs no git there, so it is not shared until a human commits and pushes it. The `tool-scopes` rule carries the full procedure.
