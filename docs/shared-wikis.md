...
`mount-init` wires the sync-embeddings hooks so the next `git pull` / checkout /
rebase both **warms the shared-category vectors** AND **(re)builds the `index.md`
nav tree** (creating the missing files on a fresh clone, refreshing them after
later pulls). That work is enqueued into a durable, per-wiki, coalescing job queue
(`state/sync-queue.sqlite`, better-sqlite3) and run **detached** — it never blocks
git, rapid branch-switching collapses to one refresh, and a run killed mid-way is
retried on the next fire. Until it completes, search still embeds lazily and the
tree is walkable from whatever indexes exist.
...
| **sync-embeddings hooks** | **shared** | on `git pull` / checkout / rebase, warm shared-category vectors AND (re)build the `index.md` nav tree — via a durable, coalescing job queue; detached, best-effort, never blocks git |
