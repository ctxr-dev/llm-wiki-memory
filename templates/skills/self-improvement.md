...
**Duplicates are the SERVER's job (discipline rule 16).** Do not run a pre-save search and do not spawn a subagent to compare candidates: the save path embeds your draft and compares it to `self_improvement` before writing. If it refuses with `duplicate-suspected`, it names the existing lesson — take that to the human as an UPDATE proposal for that id rather than saving anything. If it saves with `related`, mention the neighbour. PREFER updating the existing lesson (same `error_pattern`; a same-`name` save upserts) over a near-duplicate.
