...
- Do NOT pass `--no-llm` to silently skip LLM passes — if the provider is unavailable the orchestrator already skips them and logs once. Use `--no-llm` only when the user asks for "deterministic only" output.
