...
- The hook is gated on a 1MB plan-body cap (tunable via `hook.exitPlanModeMaxBytes` in `<data>/settings/settings.yaml`); bigger plans skip with `plan-too-large`. The hook reads the plan from the harness scratch FILE, so its size never passes through a tool argument.
- **Never inline a large document in a tool call.** MCP clients cap and truncate tool-call arguments — a ~46KB inline `save_to_dataset` payload fails to parse — and past `gate.maxInlineBodyBytes` (default 32768) the server refuses it with `inline-body-too-large`. Re-emitting a whole plan on every edit is also the slowest part of the loop by far (the engine write itself is sub-second). Write the body to a file, then save it by path: `node <clone>/scripts/cli.mjs save-leaf --file <path> --dataset plans --area=<area>` (add `--path <dir>` for a topology category such as `issues`, `--name <leaf.md>` to override the filename). `save-leaf` refuses `self_improvement`, which needs the consent-gated MCP tool.
- **To change only the frontmatter, send no body at all:** `update_document_metadata` with `select:{documentId, metadata, pin?}` patches facets / tags / priority in place. A facet change relocates the leaf and CHANGES its documentId (use the returned one); `pin:true` keeps it put; an `issues` leaf is always pinned. `status` is refused there — for a plan it is derived from the checkboxes anyway.
- **Ticking a checkbox needs neither.** Edit the leaf file; for a `*.plan.md` in your own brain wiki under interactive Claude Code the plan-sync hook re-derives `status`/`progress`, relocates an `issues` plan to its new lifecycle folder, and commits. Anywhere else (a shared-repo wiki, another client, headless) no hook fires — follow the edit with `save-leaf --file`. Because the status is derived, "never mark a plan `done` without confirmation" means **never tick the last box** before the user confirms.
