...
3. **Do not invent.** Never introduce claims not present in either body. Preserve code/commit/file provenance (commit refs, file paths, code authorship) verbatim — but NEVER introduce or keep conversational attribution of the user ("the user said", names, verbatim quotes); state facts impersonally. Preserve `**Why:**` / `**How to apply:**` structure if either input uses it. Preserve the keeper's `error_pattern` and `area` unless they are factually wrong.
